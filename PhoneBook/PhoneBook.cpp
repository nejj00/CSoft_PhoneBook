...
BOOL CPhoneBookApp::InitInstance()
{
	...
	oDBConnection.ConnectToDB(hResult, oDBPropSet);
	...
	pDocTemplate = new CMultiDocTemplate(IDR_CitiesTYPE,
		...
		RUNTIME_CLASS(CCititesView));
	if (!pDocTemplate)
		return FALSE;

	AddDocTemplate(pDocTemplate);

	pDocTemplate = new CMultiDocTemplate(IDR_PersonsTYPE,
	...
	pDocTemplate = new CMultiDocTemplate(IDR_PhoneTypesTYPE,
...
protected:
	DECLARE_MESSAGE_MAP()
public:
	afx_msg void OnNMRClickLcsPersonsPhoneNumbers(NMHDR* pNMHDR, LRESULT* pResult);
};
...
BEGIN_MESSAGE_MAP(CAboutDlg, CDialogEx)
	ON_NOTIFY(NM_RCLICK, IDC_LCS_PERSONS_PHONE_NUMBERS, &CAboutDlg::OnNMRClickLcsPersonsPhoneNumbers)
END_MESSAGE_MAP()
...
void CPhoneBookApp::PreLoadState()
{
	...
	ASSERT(bNameValid);
	GetContextMenuManager()->AddMenu(strName, IDR_POPUP_CITIES);
}
...
void CAboutDlg::OnNMRClickLcsPersonsPhoneNumbers(NMHDR* pNMHDR, LRESULT* pResult)
{
